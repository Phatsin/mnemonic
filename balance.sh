...
echo "# ── ETH via Cloudflare Ethereum Gateway ──────────────────────────────────────"
echo -n "ETH [$ETH_ADDR] : "
...
echo "# ── ETH & ERC-20 Tokens via Ethplorer ────────────────────────────────────────"
echo -n "ETH [$ETH_ADDR] : "
...
BAL_ETH=$(echo "$ETHPLORER_JSON" | jq -r '.ETH.balance')
printf "%.6f\n" "$BAL_ETH"

# 2. (The part you wanted) Loop to fetch "all tokens"
# Filter for tokens with a balance > 0
echo "$ETHPLORER_JSON" | jq -r '
    .tokens[]? | 
    select(.balance and .balance > 0) | 
    "    ↳ \(.tokenInfo.symbol)\t : \(.balance / pow(10; (.tokenInfo.decimals | tonumber)))"
'

printf "\n"
# ── SOL (Native) via Solana RPC ──────────────────────────────────────────────
echo "# ── SOL (Native) via Solana RPC ──────────────────────────────────────────────"
...
curl -s "https://api.mainnet-beta.solana.com" \
  ...
  MINT=$(echo $account | jq -r '.account.data.parsed.info.mint')
  AMOUNT=$(echo $account | jq -r '.account.data.parsed.info.tokenAmount.uiAmountString')
...
if [[ "$bal" =~ ^[0-9]+$ ]]; then
  ...
  echo "Account not found (or not activated)"
fi
...
while read -r currency balance; do
    ...
    if [[ ${#currency} -eq 40 && "$currency" != *[^0-9A-Fa-f]* ]]; then
        ...
        symbol=$(echo -n "$currency" | xxd -r -p | tr -d '\0')
    fi
...
echo "# ── XLM via Horizon (Stellar) API ────────────────────────────────────────────"
echo -n "XLM [$XLM_ADDR] : "
...
if ! echo "$XLM_JSON" | jq -e '.balances' > /dev/null; then
  echo "Account not found or no balance data."
else
  # 3. Fetch and display XLM (Native) first
  NATIVE_AMOUNT=$(echo "$XLM_JSON" | jq -r '.balances[] | select(.asset_type == "native") | .balance')
  ...
  echo "$XLM_JSON" | jq -c '.balances[] | select(.asset_type != "native")' | while read balance_entry; do
    
    BALANCE=$(echo "$balance_entry" | jq -r '.balance')
    ...
    ASSET_CODE=$(echo "$balance_entry" | jq -r '.asset_code')
